...
EPPU.prototype.positionBox = function () {
    ...
    // Changes to set the width of the box to that of the target + toggle iff the width < target
    // This improves the L&F when the popup is being used as a combo box.
    // This may (will) break some future layout scenarious, but can be modified as needed in future TODO
    var width = EP.getWidth(this.eTarget);
    if (boxWidth < width) {
        boxWidth = width + EP.getWidth(this.eToggle);
        EP.setWidth(this.eBox, boxWidth + "px");
    }
    // end mod
