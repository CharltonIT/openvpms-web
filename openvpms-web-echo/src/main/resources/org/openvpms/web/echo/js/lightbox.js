        ...
        if (messagePartElement.childNodes[i].nodeType == 1) {
            switch (messagePartElement.childNodes[i].tagName) {
                ...
                case "hidden":
                    ...
                    break;
                case "zIndex":
                    EPLightBox.MessageProcessor.processZIndex(messagePartElement.childNodes[i]);
            }
        }
    }
};
 ...
 * -----------------------------------
 */
EPLightBox.MessageProcessor.processZIndex = function (messageElement) {
    for (var itemXML = messageElement.firstChild; itemXML; itemXML = itemXML.nextSibling) {
        var elementId = itemXML.getAttribute("eid");
        var divE = document.getElementById(elementId);

        var zIndex = itemXML.getAttribute("zIndex");
        if (zIndex && divE) {
            this.zIndex = zIndex;
            if (this.zIndex != -1) {
...
EPLightBox.prototype.init = function (itemXML) {
    ...
    // BEGIN tima
    this.zIndex = itemXML.getAttribute('zIndex');
    // END tima
    ...
    if (!divE) {
        ...
        if (this.zIndex && this.zIndex != -1) {
            divE.style.zIndex = this.zIndex; // tima
        }
...
EPLightBox.prototype.showLightBox = function () {
    ...
    if (zIndex != -1 && EchoModalManager.modalElementId) {
